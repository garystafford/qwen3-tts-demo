...
def normalize_audio_db(audio, target_db=-6.0):
    """Normalize audio to target dB level (replaces sox.Transformer().norm())."""
    import numpy as np
...
class XVectorExtractor(nn.Module):
    ...
    def sox_norm(self, audio):
        return normalize_audio_db(audio, target_db=-6.0)
